...
import fr.inria.portolan.common.utils.PortolanConstants;
import fr.inria.portolan.visualization.prefuse.ui.AggregateDragControl;
import fr.inria.portolan.visualization.prefuse.ui.ToolTipsControl;
...
public class SwitchableDisplay extends Display {
	...
	protected ToolTipsControl toolTipControl;
	...
	protected void computeGroups() {
		...
		for (@SuppressWarnings("unchecked")
			...
			if (item instanceof Node) {
				...
				if (! groupValue.equals(PortolanConstants.empty)) {
	 ...
	 * @param typeField
	 */
	public void setToolTipField(String field) {
		if (toolTipControl != null) {
			this.removeControlListener(toolTipControl);
			// force release off the previous control
//			toolTipControl.setEnabled(false);
		}
		toolTipControl = new ToolTipsControl(field);
		this.addControlListener(toolTipControl);
...
//		m_vis.run(COLOR_ACTION);
	...
	public void setBalloonTreeLayout() {
		// an illimited time => nodes can not be moved
		// fixed radius to enlarge graph
		setLayout(2000, new BalloonTreeLayout(GRAPH_GROUP, 20));
	}
...
//	public void setDegreeForceDirectedLayout() {
//		// let continuous computation to play with graph :-)
//		ForceDirectedLayout layout = new DegreeForceDirectedLayout(GRAPH_GROUP);
//		setLayout(Activity.INFINITY, layout);
//	}
	
	public void setFruchtermanReingoldLayout() {
		// limit computation because moving graph
		// no fixed number of iterations
		setLayout(2000, new FruchtermanReingoldLayout(GRAPH_GROUP));
//		setLayout(Activity.INFINITY, new FruchtermanReingoldLayout(GRAPH_GROUP, 10));
	}
	
	public void setLinLogLayout() {
		// limit computation because moving graph
		// no fixed number of iterations
//		setLayout(Activity.INFINITY, new LinLogLayout(GRAPH_GROUP));
		setLayout(2000, new LinLogClustersLayout(GRAPH_GROUP));
	}
	
	public void setNodeLinkTreeLayout() {
		// an illimited time => nodes can not be moved
		setLayout(2000, new NodeLinkTreeLayout(GRAPH_GROUP));
	}
	
	public void setRadialTreeLayout() {
		// an illimited time => nodes can not be moved
		// force the radius
		setLayout(2000, new RadialTreeLayout(GRAPH_GROUP, 200));
	}
